...
word_list = ["about","above","abuse","actor","acute","admit","adopt","adult","after","again","agent",
...
def start_game():
    ...
    user_guess[int(user_id)] = []
    user_correct[int(user_id)] = list("" for i in range(0,5))
...
@app.route('/guess/<user_id>/<letter>')
def start_guessing(user_id,letter):
    word = word_list[int(user_id)%499]
    print(word)
    guess_list = user_guess[int(user_id)]
    correct_list = user_correct[int(user_id)]
    print(correct_list)

    guess_list+=[letter]
    user_guess[int(user_id)] = guess_list
    ...
    if game_over:
        ...
        stat = 200
        user_guess[int(user_id)] = []
        user_correct[int(user_id)] = ["","","","",""]
    elif match:
        message = "There is an " + letter.upper() +"!"
        stat = 200
    else:
        message = "There is no " + letter.upper() +"!"
        stat = 400
    ...
    js = json.dumps(data)
    resp = Response(js, status=stat, mimetype='application/json')
    return resp
